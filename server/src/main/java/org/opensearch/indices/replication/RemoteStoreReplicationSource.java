...
public class RemoteStoreReplicationSource implements SegmentReplicationSource {
    ...
    public void getSegmentFiles(
        ...
        try {
            ...
            List<StoreFileMetadata> toDownloadSegmentMetadata = new ArrayList<>();
            ...
            if (remoteSegmentMetadata != null) {
                try {
                    ...
                    for (StoreFileMetadata fileMetadata : filesToFetch) {
                        ...
                        toDownloadSegmentMetadata.add(fileMetadata);
                        toDownloadSegmentNames.add(file);
                    }
                    indexShard.getFileDownloader().download(storeDirectory, remoteDirectory, toDownloadSegmentNames);
                    logger.debug("Downloaded segment files from remote store {}", toDownloadSegmentMetadata);
                } finally {
                    indexShard.store().decRef();
                    indexShard.remoteStore().decRef();
                }
            }
            listener.onResponse(new GetSegmentFilesResponse(toDownloadSegmentMetadata));
        } catch (Exception e) {
            listener.onFailure(e);
        }
    }
